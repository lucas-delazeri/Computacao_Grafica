...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        ...
        if (key == GLFW_KEY_LEFT) {
            ...
            if (rPressed) rotation.y -= 5.0f; // Rotação no eixo Y ao girar horizontalmente
        ...
        if (key == GLFW_KEY_UP) {
            ...
            if (ePressed || sPressed) scaleVal.y += 0.1f;
        }
        ...
        if (key == GLFW_KEY_PAGE_UP) {
            ...
            if (ePressed || sPressed) scaleVal.z += 0.1f;
        }
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "Exercicios OpenGL", NULL, NULL);
    ...
    // Vértices do Cubo (Exercício 2). Para o Quadrado (Exercício 1), use apenas os primeiros 6 vértices
// Cada linha possui: Position (X, Y, Z), Color (R, G, B)
float vertices[] = {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Desenha o Cubo (36 vértices). Para o Quadrado, altere para glDrawArrays(GL_TRIANGLES, 0, 6);
